...
public class AlienTilesPanel  extends JPanel implements Runnable
{
	...
	//to diplay the title.help screen
	...
	// create the game world, the player, and aliens
	{
		...
		// the 4th alien is posted at an illegal tile location (34,34)
	...
	//handles termination, help, and game-play keys
	{
		...
		if(!isPaused && !gameOver) {
			...
			if(keyCode == KeyEvent.VK_NUMPAD7)
				...
				player.move(TiledSprite.NE);  // move north west
...
/* The frames of the animation are drawn inside the while loop. */
{
	...
	while(running) {
		 ...
		 * without rendering it, to get the updates/sec nearer
		 * the required FPS. */
		int skips = 0;
...
if (dbImage == null) {
	System.out.println("dbImage isr null");
	return;
} 
else 
	dbg = dbImage.getGraphics();
 
// light a blue background
