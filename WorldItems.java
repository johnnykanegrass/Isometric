...
// max pixel width/height of a tile (inlcuding space to next tile)
	private int tileWidth, tileHeight;
	
// 'start of first even row' coordinate
	/* The coordinate is ithe top-left corner of the rectangle
	 ...
	 * relevant row coord information depending on if it is in an even or odd numbered row. The pixel dimensions of a tile are also needed,
	...
	*/
	...
	/* Insert the toc item into the list in row oder. Within a row,
	the items are ordered by their column Position.class */
	{
		TileOccupier item;
		Point itemPt;
		int i = 0;
	 ...
	 * list. This method is a variant of addItem(). */
	{
	 ...
	 * Conpmared pt with the _square_ of its distance to each pickup.
	 * The squared distance is used to avoid problems with negative
	 * distances.
	 * */
	{
		double minDist = 1000000; // dunmmy large value ( a hack)
		Point minPoint = null;
		double dist;
		TileOccupier item;
		for(int i=0; i < items.size(); i++) {
